...
```php
zillowGetMultifamilyBuilding($url): mixed
```
...
try {
    $result = $apiInstance->zillowGetMultifamilyBuilding($url);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ZillowApi->zillowGetMultifamilyBuilding: ', $e->getMessage(), PHP_EOL;
}
```
